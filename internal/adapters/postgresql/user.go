...
func (u *userStorage) AuthUser(ctx context.Context, user db_dto.AuthUserDTO) (user_usecase.ReturnUser, error) {
	var dto user_usecase.ReturnUser
	sql, args, err := sq.Select("id", "is_seller").From(User).Where("(email = $1 OR username = $2) AND passwordHash = $3", user.Email, user.Username, user.PasswordHash).ToSql()
	if err != nil {
		return dto, apperror.NewAppError(err, "incorrect data entered", err.Error(), "US-000004")
	}
	if err = u.client.QueryRow(ctx, sql, args...).Scan(&dto.UserID, &dto.IsSeller); err != nil {
		if err == pgx.ErrNoRows {
			return dto, apperror.NewAppError(err, "invalid login or password entered", "", "US-000009")
		}
		if pgErr, ok := err.(*pgconn.PgError); ok {
			newErr := fmt.Errorf("SQL Error: %s, Detail: %s, Where: %s, Code: %s, SQLState: %s", pgErr.Message, pgErr.Detail, pgErr.Where, pgErr.Code, pgErr.SQLState())
			u.logger.Errorln(newErr)
			return dto, apperror.NewAppError(err, "SQL error", pgErr.Message, "US-000005")
		}
		return dto, apperror.NewAppError(err, "SQL error", err.Error(), "US-000005")
	}
	return dto, nil
}
...
func (u *userStorage) GetUserByRefreshToken(ctx context.Context, user db_dto.GetUserByRefreshTokenDTO) (user_usecase.ReturnUser, error) {
	sql, args, err := sq.Select("id", "is_seller").From(User).Where("session->>'RefreshToken' = $1", user.Token).ToSql()
	var dto user_usecase.ReturnUser
	if err != nil {
		return dto, apperror.NewAppError(err, "incorrect data entered", err.Error(), "US-000004")
	}
	if err = u.client.QueryRow(ctx, sql, args...).Scan(&dto.UserID, &dto.IsSeller); err != nil {
		if err == pgx.ErrNoRows {
			return dto, apperror.NewAppError(err, "invalid token", "", "TJ-000004")
		}
		if pgErr, ok := err.(*pgconn.PgError); ok {
			newErr := fmt.Errorf("SQL Error: %s, Detail: %s, Where: %s, Code: %s, SQLState: %s", pgErr.Message, pgErr.Detail, pgErr.Where, pgErr.Code, pgErr.SQLState())
			u.logger.Errorln(newErr)
